...
class Model_tamu extends MY_Model {
	public function __construct()
	{
		$this->load->database(); //Siapkan saja inijangan sampai error nanti, tapi kan semestinya instance db sudah global
		$table = "tamu"; //ini karena dikatakan sebagai kelas Model_user, yaitu kelas untuk CRUID tabel user
		$kolom_cookie = "cookie";
    ...
    {
		$query = $this->db->get_where('tamu', $data);
	...
	public function save($data)
	{
		...
		$user = [
			...
			'email' => $data['email'],
			'bidang' => (isset($data['bidang'])) ? $data['bidang'] : 'unknown',
